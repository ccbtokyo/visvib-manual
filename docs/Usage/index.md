...
> [!NOTE]
> ソフトウェア上では黒鍵の音程が`A#4`や`D#4`などシャープで表記されてますが、トーンチャイムでは`B♭4`や`E♭4` などフラット表記になっているので、間違えないようにしましょう。また、音名をテキストで入力することも可能です（半角モードで入力してください。Enterキーで確定すると値が反映されます。）
...
トーンチャイムを強めに叩いた時に、Light Brightnessが右端までいかなければ、上の方にある、Range-Inputというスライダーを、少しずつ **下げて** 感度を調整します。逆に、トーンチャイムを叩いたらスライダーが右端に張り付いて、しばらくしてから下がってくるような場合は、Range-Inputを**上げて**感度を調整します。
